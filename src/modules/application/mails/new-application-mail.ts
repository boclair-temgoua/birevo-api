...
export const newApplicationMail = async (options: { item: any }) => {
  const { item } = { ...options };
  const output = `
  ...
  <h2 style="margin-bottom:20px">New application key</h2>

  <span style="font-size:16px">
      A new API Key <strong>${
        item?.name
      }</strong> was created on your account at <strong>${moment(
    item?.createdAt,
  ).format('LLLL')}</strong> .<br><br>
      If you or one of your users did not create this key, please revoke the key, 
      change your password, and contact Render support immediately at.
      You can view your <a style="text-decoration:none"
          href="${configurations.datasite.urlClient}/applications"
          target="_blank">Application</a>
      to review all of your API keys.
  </span><br/><br/>
  ...
  <a href="${
    configurations.datasite.urlClient
  }/applications" style="box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol'; position: relative; color: #3869d4;">
  ${configurations.datasite.urlClient}/applications
  </a></span></p>
  ...
  await NodeMailServiceAdapter({
    to: [`${item.email}`],
    subject: `${configurations.datasite.name} - New API Key Created`,
    html: output,
  });
};
