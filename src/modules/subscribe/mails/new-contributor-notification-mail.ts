...
export const newContributorNotificationMail = async (options: {
  item: any;
}) => {
  const { item } = options;
  const output = `
  ...
  <h2 style="margin-bottom:20px">Your team is waiting for you to join them</h2>

  <span style="font-size:16px">
      ${item?.userGuest?.profile?.fullName} has invited you to collaborate on ${
    item?.userGuest?.organizationInUtilization?.name
  }
  </span><br/><br/>
  ...
  }/organizations/user">Join the team</a>
  </td>
  </tr>
  </table><br/><br/>
  ...
  If you’re having trouble clicking the "Join the team" button, copy and paste the URL below into your web browser: <span class="break-all" style="box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol'; position: relative; word-break: break-all;">
  <a href="${
    configurations.datasite.urlClient
  }/organizations/user" style="box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol'; position: relative; color: #3869d4;">
  ${configurations.datasite.urlClient}/organizations/user
  </a></span></p>
  ...
  <td style="font-family: Montserrat, -apple-system, 'Segoe UI', sans-serif; font-size: 12px; padding-left: 48px; padding-right: 48px; --text-opacity: 1; color: #eceff1; color: rgba(236, 239, 241, var(--text-opacity));">
    <p style="--text-opacity: 1; color: #263238; color: rgba(38, 50, 56, var(--text-opacity));">
      ...
      <a href="${
        configurations.datasite.urlClient
      }/organizations/user" class="hover-underline" style="--text-opacity: 1; color: #7367f0; color: rgba(115, 103, 240, var(--text-opacity)); text-decoration: none;">Terms of Use</a> and
      <a href="${
        configurations.datasite.urlClient
      }/organizations/user" class="hover-underline" style="--text-opacity: 1; color: #7367f0; color: rgba(115, 103, 240, var(--text-opacity)); text-decoration: none;">Privacy Policy</a>.
    </p>
  </td>
  </tr>
  ...
  }/organizations/user. All rights reserved.</p>
  ...
  await NodeMailServiceAdapter({
    to: [`${item?.userContributor?.email}`],
    subject: `${configurations.datasite.name} - Welcome to ${item?.userGuest?.organizationInUtilization?.name}`,
    html: output,
  });
};
